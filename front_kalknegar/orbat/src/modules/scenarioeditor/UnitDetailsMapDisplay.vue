...
const ringMenuItems = computed((): MenuItemData<RangeRingAction>[] => [
  ...
  { label: "حذف", action: RangeRingActions.Delete, disabled: props.isLocked },
]);

function addRangeRing() {
  const defaultRing: RangeRing = {
    name: "حلقه برد جدید-3 " + nanoid(3),
    range: 20,
    uom: "km",
    group: null,
  };
...
function getGroupName(ring: RangeRing) {
  if (!ring.group) return "هیچکدام";
  return store.state.rangeRingGroupMap[ring.group]?.name || "هیچکدام";
}
...
<template>
  ...
  <div class="mt-4 flex items-center justify-between">
    <PanelHeading>حلقه‌های برد</PanelHeading>
          ...
          <td colspan="5">
            ...
            >
              <InputGroup
                ...
                :disabled="isLocked"
              />
              <InputGroupTemplate label="برد" v-slot="{ id }" class="col-span-1">
                <div class="relative rounded-md shadow-xs">
                  ...
                  <div class="absolute inset-y-0 right-0 flex items-center">
                    <label for="currency" class="sr-only">واحد</label>
                    <select
                      id="range"
                      name="range"
              ...
              <div class="col-span-2 flex items-center justify-between">
                ...
                <div>
                  <Button type="submit" variant="secondary" size="sm">بروزرسانی</Button>
                  <Button
                    variant="outline"
                    size="sm"
                    class="ml-2"
