...
const props = withDefaults(
  ...
  {
    heading: "افزودن دسته‌بندی تدارکات جدید",
  },
);
...
const supplyClasses = computed(() => {
  ...
  return [{ label: "مشخص نشده", value: "" }, ...classes];
});
...
<template>
  ...
  >
    ...
    <section class="mt-4 grid grid-cols-2 gap-6">
      <InputGroup autofocus label="نام" required v-model="form.name" />
      <SimpleSelect label="کلاس" v-model="form.supplyClass" :items="supplyClasses" />
      <InputGroup class="" label="توضیحات" v-model="form.description" />
      <SimpleSelect
        label="واحد اندازه‌گیری/صادرات"
        v-model="form.uom"
        :items="supplyUnits"
      />
    </section>
