<template>
  <FloatingPanel class="pointer-events-auto flex items-center space-x-0.5 rounded-md p-1">
    <p class="text-muted-foreground px-2 text-sm font-medium">اندازه‌گیری</p>
    <MainToolbarButton
      title="طول"
    ...
    <MainToolbarButton
      title="نمایش دایره محدوده"
      @click="showCircle = !showCircle"
      :active="showCircle"
    >
      <ShowCircleIcon class="size-5" />
    </MainToolbarButton>
    <MainToolbarButton title="تغییر حالت چسباندن" @click="snap = !snap" :active="snap">
      <SnapIcon class="size-5" />
    </MainToolbarButton>
    <MainToolbarButton title="پاک کردن اندازه‌گیری‌ها" @click="clear()">
      <TrashIcon class="size-5" />
    </MainToolbarButton>

    <MainToolbarButton title="تغییر حالت نوار ابزار" @click="store.clearToolbar()">
