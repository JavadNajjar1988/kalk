<template>
  <InlineFormPanel @close="$emit('close')" title="ویرایش اطلاعات گروه">
    <form @submit.prevent="onFormSubmit" class="space-y-4">
      <InputGroup label="نام گروه" v-model="form.name" :id="focusId" />
      ...
      <div class="flex justify-end space-x-2">
        ...
        <PlainButton @click="$emit('close')">لغو</PlainButton>
      </div>
    </form>
  </InlineFormPanel>
</template>
...
let form = ref<Form>({ name: "واحدها", symbolOptions: {} });
