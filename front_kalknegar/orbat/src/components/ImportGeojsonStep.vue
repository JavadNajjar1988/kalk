...
const computedColumns = computed((): (ColumnDef<GeoJSONFeature, any> | false)[] => {
  ...
  return [
    { accessorKey: "geometry.type", id: "geometryType", header: "هندسه" },
    ...
    {
      accessorFn: (f) => f.properties?.[nameColumn.value] ?? "فیچر",
      id: "name",
      header: "نام",
    },
    { header: "ویژگی‌های فیچر", columns: [...items] },
  ];
});
    ...
    <form @submit.prevent="onLoad" class="mt-4 flex max-h-[80vh] flex-col">
      <fieldset class="flex items-center gap-x-10">
        <p class="flex-none text-sm leading-6 font-semibold text-gray-900">
          وارد کردن GeoJSON به عنوان
        </p>
        <MRadioGroup class="flex w-full gap-10">
          <InputRadio v-model="importMode" value="features">ویژگی‌های سناریو</InputRadio>
          <InputRadio v-model="importMode" value="units">واحدها</InputRadio>
        </MRadioGroup>
      </fieldset>
      <div class="">
        <p class="mt-4 text-sm leading-6 text-gray-600">
          فیچرهایی که می‌خواهید وارد کنید را انتخاب کنید
        </p>
        <section class="mt-4">
          <DataGrid
          ...
          <AlertWarning
            ...
            title="هیچ هندسه نقطه‌ای یافت نشد"
            class="mt-4"
          >
            برای وارد کردن واحد باید دارای هندسه نقطه‌ای باشد.
          </AlertWarning>
        </section>
        ...
        <section class="mt-4 grid gap-4 sm:grid-cols-2">
          <SimpleSelect
            v-if="isFeatureMode"
            label="لایه"
            description="ویژگی‌ها به کدام لایه اضافه شوند؟"
      ...
      <footer class="flex shrink-0 items-center justify-end space-x-2 pt-4">
        <Button type="submit" size="sm">وارد کردن</Button>
