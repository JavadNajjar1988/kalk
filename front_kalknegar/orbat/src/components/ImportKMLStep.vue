    ...
    <form @submit.prevent="onLoad" class="flex max-h-[80vh] flex-col">
      <div v-if="inputOption === 'imageLayer'">
        <section class="my-4 space-y-4">
          <InputGroup label="نام لایه برداری" v-model="form.layerName" />
          <InputCheckbox label="استخراج استایل‌های KML" v-model="form.extractStyles" />
        </section>

        <AlertWarning title="هشدار"
          >لایه‌های KML در حال حاضر فقط تا زمانی که سناریو باز است قابل مشاهده هستند. به عنوان بخشی از سناریو ذخیره نمی‌شوند.
        </AlertWarning>
      </div>
