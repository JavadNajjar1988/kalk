...
<template>
  <div class="px-6 py-4">
    <h3 class="-my-3 flex w-full items-center justify-between py-3">
      ...
      >
        <span class="min-w-0 flex-auto truncate text-right font-bold text-gray-900">
          <slot name="label" :open="isOpen">{{ label }}</slot>
        </span>
      </button>
      <span class="relative mr-6 flex shrink-0 items-center">
        <slot name="right" :open="isOpen"></slot>
        <ChevronDownIcon
          class="mr-2 h-6 w-6 flex-none rotate-180 transform text-gray-500 transition-transform group-hover:text-gray-900"
          :class="{
            'rotate-0': isOpen,
          }"
        />
