...
const eraseSegments = (geometry) => {
  ...
  switch (geometry.getType()) {
    ...
    case "MultiPolygon":
      return geometry
        .getCoordinates()
        .flatMap((polygon) => polygon.flatMap(lineSegments));
...
const brushMapRadius = (feature, map, size) => {
  ...
  if (geometry?.getType?.() === "Point") {
    return resolution * brushCursorRadius(size);
  }
  const baseLine = geometry ? toFadeBaseLine(TS.read(geometry)) : null;
  const length = baseLine?.getLength?.() ?? 0;
  if (!Number.isFinite(length) || length <= 0) return 0;
...
export default (options) => {
  ...
  const pickOnFeature = (feature, event) => {
    if (feature?.getGeometry?.()?.getType?.() === "Point") {
      return { coordinate: [...event.coordinate], t: 0.5 };
    }
  ...
  const persistSpatialFade = (feature, coordinates, segment) => {
    if (!coordinates.length || !cutGestureId) return;
    const spatialFades = mergeSpatialCut(feature.get("spatialFades"), {
      coordinates,
      radius: brushMapRadius(feature, map, brushSize),
      gesture: `${cutGestureId}:${segment}:${feature.getId()}`,
    });
    feature.set("spatialFades", spatialFades);
    stageWrite(feature, "spatialFades");
    feature.commit?.();
  };
  ...
  const applySpatialHits = (hits) => {
    ...
    hits.forEach((hit) => {
      ...
      if (mode === "cut") persistSpatialCut(hit.feature, coordinates, segment);
      else persistSpatialFade(hit.feature, coordinates, segment);
      emitter.emit("ui/erase/applied", { mode });
      cutTargets.set(id, {
        feature: hit.feature,
  ...
  function handleEvent(event) {
    ...
    if (isBrushingMove) {
      ...
      if (pick) {
        applySpatialHits([{ feature: brushFeature, ...pick }]);
