...
describe("eraseInteraction", () => {
  it("stores a local mask when brushing a single-point tactical symbol", () => {
    const feature = new Feature(new Point([10, 20]));
    feature.setId("feature:point");
    feature.set("sidc", "SFGPUCI----K---");
    const state = {
      "feature:point": {
    ...
    const store = {
      update: (keys, updater) => keys.forEach((key) => (state[key] = updater(state[key]))),
    };
    const emitter = new EventEmitter();
    const map = makeMap(feature, 2);
    const interaction = eraseInteraction({ services: { store, emitter }, map });

    emitter.emit("ERASE_CUT_START", { brushSize: 3 });
    interaction.handleEvent(pointerEvent(map, "pointerdown", [12, 21]));
    interaction.handleEvent(pointerEvent(map, "pointerup", [12, 21], 0));

    expect(state["feature:point"].properties.spatialCuts).toHaveLength(1);
    expect(state["feature:point"].properties.spatialCuts[0]).toMatchObject({
      coordinates: [[12, 21]],
      radius: 22,
    });
  });
  ...
  it("persists fade zones while brushing a line feature", () => {
    ...
    expect(state["feature:test"].properties.spatialFades).toHaveLength(1);
    expect(state["feature:test"].properties.spatialFades[0]).toMatchObject({
      coordinates: [
        [2, 0],
        [5, 0],
      ],
  ...
  it("uses the visible brush size for hit detection even when map hit tolerance is small", () => {
    ...
    expect(state["feature:test"].properties.spatialFades).toBeDefined();
  });
  ...
  it("does not connect separate cut segments when the pointer leaves the symbol", () => {
    ...
    expect(
      state["feature:test"].properties.spatialCuts.map((cut) => cut.coordinates),
    ).toEqual([[[2, 0]], [[8, 0]]]);
  });
  ...
  it("finishes a stroke when the primary mouse button is no longer pressed", () => {
    ...
    expect(state["feature:test"].properties.spatialFades[0].coordinates).toEqual([
      [2, 0],
      [5, 0],
    ]);
  });
