...
export default (options) => {
  ...
  emitter.on("hostility/selected", ({ code }) => {
    selectedHostility = code || "F";
  });

  // Listen for status changes
  emitter.on("status/selected", ({ code }) => {
    selectedStatus = code || "P";
  });
  ...
  const drawstart =
    ...
    ({ feature }) => {
      ...
      const center = view?.getCenter?.() ??
        feature.getGeometry()?.getFirstCoordinate?.() ?? [0, 0];
      const metersPerPixel = projection
        ? getPointResolution(projection, resolution, center, "m")
        : resolution;
      feature.set("symbolInstanceId", uuid());
      feature.set("renderReference", {
        version: 1,
        renderer: "tactical-2525c",
        authoredResolution: resolution,
        authoredScale: (metersPerPixel * 96) / 0.0254,
        pointSizeMeters: metersPerPixel * 30,
      });
  ...
  const drawend =
    ...
    ({ feature }) => {
      ...
      try {
        emitter.emit("ui/tactical/draw-complete");
      } catch {}
      cancel();
    };

  emitter.on("command/draw/cancel", ({ originatorId }) => {
    if (originatorId !== ORIGINATOR_ID) {
      cancel();
    }
  });
...
const geometries = [
  ...
  /* LineString. */
  {
    ...
    options: (descriptor) =>
      withMaxPoints({ type: GeometryType.LINE_STRING }, descriptor),
  },
  ...
  {
    match: ({ geometry }) =>
      geometry.layout === "fan" && Number.parseInt(geometry.maxPoints) === 3,
    options: () => ({ type: GeometryType.POINT }),
  ...
  {
    match: ({ geometry }) =>
      geometry.layout === "fan" && Number.parseInt(geometry.maxPoints) === 2,
    options: () => ({ type: GeometryType.POINT }),
    complete: (map, feature) => {
      const resolution = map.getView().getResolution();
  ...
  {
    match: ({ geometry }) =>
      geometry.layout === "circle" && Number.parseInt(geometry.maxPoints) === 2,
    options: () => ({ type: GeometryType.POINT }),
    complete: (map, feature) => {
      const resolution = map.getView().getResolution();
      const radius = resolution * 50;
