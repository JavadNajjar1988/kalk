...
const specifics = ($) => {
  $.simplifiedGeometry = Signal.link(
    _simplifiedGeometry,
    [$.geometry, $.renderCenterResolution],
    { equals: keyequals() },
  );
  $.jtsSimplifiedGeometry = $.simplifiedGeometry.ap($.read);
  $.lineSmoothing = $.effectiveStyle.map(_lineSmoothing);
  $.smoothenedGeometry = Signal.link(_smoothenedGeometry, [
    $.simplifiedGeometry,
    $.lineSmoothing,
  ]);
  ...
  $.labels = $.parameterizedSIDC.map(_labels(labels)).ap($.placement);
