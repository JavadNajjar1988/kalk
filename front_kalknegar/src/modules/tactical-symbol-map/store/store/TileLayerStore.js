...
import * as TileService from './tileServiceAdapters.js'
...
TileLayerStore.prototype.tileLayers = async function () {
  // Ensure bootstrap is called first
  ...
  // Fallback: if preset is still undefined or empty, return empty layer group
  if (!preset || !Array.isArray(preset) || preset.length === 0) {
    this.layerCollection = new Collection([])
