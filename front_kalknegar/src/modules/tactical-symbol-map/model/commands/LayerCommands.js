import { readEntries } from '../../renderer/Clipboard.js'
...
const SetDefaultLayer = function (services) {
  ...
  this.toolTip = 'تنظیم لایه انتخابشده به عنوان پیشفرض'
...
const SelectTilePreset = function (services) {
  ...
  this.toolTip = 'تغییر وضعیت نمایش نقشههای پسزمینه'
  this.path = 'mdiMap'
}
...
const ExportLayer = function (services) {
  ...
  this.ipcRenderer = services.ipcRenderer
  this.path = 'mdiExport'
  this.toolTip = 'خروجی گرفتن از لایه انتخابشده به پرونده'
...
ExportLayer.prototype.execute = async function () {
  ...
  const content = {
    contentType: 'application/json;vnd=kalk',
    entries
  }

  const layerId = this.selected()[0]
  const layer = await this.store.value(layerId)
  // Web: download JSON directly. Electron: delegate to main process via IPC.
  const done = downloadJSON(layer.name, content)
  if (!done && this.ipcRenderer?.send) {
    this.ipcRenderer.send('EXPORT_LAYER', layer.name, content)
  }
}
