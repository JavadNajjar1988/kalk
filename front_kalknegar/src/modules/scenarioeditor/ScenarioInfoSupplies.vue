...
const columns: ColumnDef<NSupplyCategory>[] = [
  ...
  { id: "class", header: "کلاس", accessorFn: (f) => getSupplyClass(f, store.state) },
  { id: "unit", header: "واحد", accessorFn: (f) => getUom(f, store.state), size: 80 },
  {
    id: "description",
    header: "توضیحات",
  ...
  store.groupUpdate(() => {
    selectedSupplies.value.forEach((e) => {
      ...
      if (!success) {
        send({
          type: "error",
          message: `${e.name}: نمی‌توان دسته‌بندی تدارکات در حال استفاده را حذف کرد.`,
        });
...
function onAddSubmit(formData: Omit<NSupplyCategory, "id">) {
  ...
  if (supplies.value.find((e) => e.name === formData.name)) {
    send({
      type: "error",
      message: "دسته‌بندی تدارکات با این نام قبلاً وجود دارد.",
    });
    return;
  }
...
<template>
  <div class="">
    <TableHeader description="فهرست دسته‌بندی‌های تدارکات موجود در این سناریو.">
    </TableHeader>
    <ToeGridHeader
      v-model:editMode="editMode"
      v-model:addMode="showAddForm"
      editLabel="ویرایش دسته‌بندی تدارکات"
      ...
      <template #inline-form="{ row }">
        <InlineFormWrapper class="pr-6">
          <AddSupplyCategoryForm
            ...
            @cancel="cancelEdit()"
            heading="ویرایش دسته‌بندی تدارکات"
            showNextToggle
          />
        </InlineFormWrapper>
      </template>
    ...
    <p v-else class="prose prose-sm dark:prose-invert">
      از دکمه <kbd>افزودن</kbd> برای افزودن دسته‌بندی‌های تدارکات به این سناریو استفاده کنید.
    </p>
  </div>
</template>
