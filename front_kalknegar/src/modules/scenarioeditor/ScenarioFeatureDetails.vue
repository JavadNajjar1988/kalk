...
const tabList = computed(() =>
  uiStore.debugMode
    ? ["سبک", "جزئیات", "وضعیت", "تبدیل", "اشکال‌زدایی"]
    : ["سبک", "جزئیات", "وضعیت", "تبدیل"],
);
  ...
  <div>
    ...
    <header class="">
      <div v-if="isMultiMode" class="mt-6 mb-2 flex items-center justify-between">
        <p class="font-medium">{{ selectedFeatureIds.size }} ویژگی انتخاب شده</p>
      ...
      <nav class="flex items-center justify-between">
        <div class="flex items-center">
          ...
          />
          <IconButton @click="doZoom()" title="بزرگنمایی به ویژگی">
            <ZoomIcon class="size-5" />
          </IconButton>
          <IconButton @click="showStylePanel()" title="تغییر سبک ویژگی">
            <StyleIcon class="size-5" />
          </IconButton>
          <IconButton title="ویرایش داده‌های ویژگی" @click="toggleEditMode()">
            <EditIcon class="size-5" />
          </IconButton>
          <IconButton title="افزودن/تغییر تصویر" @click="toggleEditMediaMode()">
