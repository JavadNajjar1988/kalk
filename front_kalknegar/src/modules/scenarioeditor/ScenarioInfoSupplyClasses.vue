...
const columns: ColumnDef<NSupplyClass>[] = [
  { id: "name", header: "نام", accessorKey: "name" },
...
function onAddSubmit(formData: Omit<NSupplyClass, "id">) {
  ...
  if (supplyClasses.value.find((e) => e.name === formData.name)) {
    send({
      type: "error",
      message: "کلاس تدارکات با این نام قبلاً وجود دارد.",
    });
    return;
  }
  ...
  scn.store.groupUpdate(() => {
    selectedItems.value.forEach((e) => {
      ...
      if (!success) {
        send({
          type: "error",
          message: `${e.name}: نمی‌توان کلاس تدارکات در حال استفاده را حذف کرد.`,
        });
...
<template>
  <div class="">
    <TableHeader description="فهرست کلاس‌های تدارکات موجود در این سناریو." />
    <ToeGridHeader
      v-model:editMode="editMode"
      v-model:addMode="showAddForm"
      editLabel="ویرایش کلاس‌های تدارکات"
      ...
      <template #inline-form="{ row }">
        <InlineFormWrapper class="pr-6">
          <AddNameDescriptionForm
            ...
            @cancel="cancelEdit()"
            heading="ویرایش کلاس تدارکات"
          />
        </InlineFormWrapper>
      </template>
    </ToeGrid>
    <p v-else class="prose prose-sm dark:prose-invert">
      از دکمه <kbd>افزودن</kbd> برای افزودن کلاس‌های تدارکات به این سناریو استفاده کنید.
    </p>
  </div>
</template>
