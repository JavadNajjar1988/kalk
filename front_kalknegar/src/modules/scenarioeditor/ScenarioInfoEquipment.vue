...
function onAddSubmit(formData: Omit<NEquipmentData, "id">) {
  ...
  if (equipment.value.find((e) => e.name === formData.name)) {
    send({
      type: "error",
      message: "دسته‌بندی تجهیزات با این نام قبلاً وجود دارد.",
    });
    return;
  }
  ...
  scn.store.groupUpdate(() => {
    selectedItems.value.forEach((e) => {
      ...
      if (!success) {
        send({
          type: "error",
          message: `${e.name}: نمی‌توان دسته‌بندی تجهیزات در حال استفاده را حذف کرد.`,
        });
...
<template>
  <div class="">
    <TableHeader
      description="فهرست دسته‌بندی‌های تجهیزات موجود در این سناریو."
    />
    <ToeGridHeader
      v-model:editMode="editMode"
      v-model:addMode="showAddForm"
      editLabel="ویرایش تجهیزات"
      ...
      <template #inline-form="{ row }">
        <InlineFormWrapper class="pr-6">
          <AddNameDescriptionForm
            ...
            @cancel="cancelEdit()"
            heading="ویرایش دسته‌بندی تجهیزات"
            showNextToggle
          />
        </InlineFormWrapper>
      </template>
    ...
    <p v-else class="prose prose-sm dark:prose-invert">
      از دکمه <kbd>افزودن</kbd> برای افزودن دسته‌بندی‌های تجهیزات به این سناریو استفاده کنید.
    </p>
  </div>
</template>
