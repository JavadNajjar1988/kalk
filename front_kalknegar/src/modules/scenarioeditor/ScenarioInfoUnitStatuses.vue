...
function onAddSubmit() {
  ...
  if (statuses.value.find((e) => e.name === addForm.value.name)) {
    send({
      type: "error",
      message: "وضعیت واحد با این نام قبلاً وجود دارد.",
    });
    return;
  }
  ...
  switch (action) {
    ...
    case "delete":
      ...
      if (!success) {
        send({
          type: "error",
          message: "نمی‌توان وضعیت واحد در حال استفاده را حذف کرد.",
        });
      }
      break;
  }
...
<template>
  <div class="prose dark:prose-invert max-w-none">
    <TableHeader description="فهرست وضعیت‌های واحد موجود در این سناریو.">
      <Button variant="outline" @click="store.toggleAddEquipment()">
        {{ store.showAddEquipment ? "مخفی کردن فرم" : "افزودن" }}
      </Button>
    </TableHeader>
    <form
    ...
    >
      ...
      <div class="col-span-2 flex items-start gap-3">
        <InputGroup class="" label="توضیحات" v-model="addForm.description" />
