<template>
  <h3 class="mt-6 font-medium text-gray-900">وضعیت واحد</h3>
  <div class="flex items-center justify-between">
    <span class="text-sm">تغییر</span>
  ...
  <ul class="mt-2 divide-y divide-gray-200 border-t border-b border-gray-200">
    <li v-if="unit.location" class="relative flex items-center py-4">
      <div class="flex min-w-0 flex-auto flex-col text-sm">
        <span class="font-medium text-gray-500">موقعیت اولیه</span>
        <CoordinateInput
    ...
    >
      <div class="flex min-w-0 flex-auto flex-col text-sm">
        ...
        <div class="mt-1 flex gap-1">
          ...
          >
          <span v-if="s.update?.equipment" class="badge">تجهیزات</span>
          <span v-if="s.update?.personnel" class="badge">پرسنل</span>
          <span v-if="s.update?.supplies" class="badge">تدارکات</span>
          <span v-if="s.diff?.equipment" class="badge">±تجهیزات</span>
          <span v-if="s.diff?.personnel" class="badge">±پرسنل</span>
          <span v-if="s.diff?.supplies" class="badge">±تدارکات</span>
        </div>
      </div>

      <div class="relative flex flex-0 items-center space-x-0">
        <IconButton title="رفتن به زمان و مکان" @click="changeToState(s)">
...
const menuItems = computed((): MenuItemData<StateAction>[] => [
  { label: "حذف", action: "delete", disabled: props.isLocked },
  { label: "تکرار", action: "duplicate", disabled: props.isLocked },
  { label: "تغییر زمان", action: "changeTime", disabled: props.isLocked },
  { label: "ویرایش عنوان", action: "editTitle", disabled: props.isLocked },
  { label: "ویرایش موقعیت", action: "editLocation", disabled: props.isLocked },
  { label: "پاک کردن موقعیت", action: "clearLocation", disabled: props.isLocked },
  {
    label: "تبدیل به موقعیت اولیه",
...
const initialMenuItems = computed((): MenuItemData<StateAction>[] => [
  { label: "حذف", action: "delete", disabled: props.isLocked },
  { label: "ویرایش موقعیت اولیه", action: "editLocation", disabled: props.isLocked },
]);
...
async function handleChangeSymbol() {
  const newSidcValue = await getModalSidc(props.unit.sidc, {
    title: `تغییر نماد در ${formatDateString(
