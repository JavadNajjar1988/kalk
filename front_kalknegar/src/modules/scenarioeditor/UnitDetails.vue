    ...
    <header class="-mx-4 px-2 pt-2">
      ...
      <div v-else>
        <div class="flex items-center justify-between">
          <p class="font-medium">{{ selectedUnitIds.size }} واحد انتخاب شده</p>
      ...
      <nav class="-mt-4 mb-4 flex items-center justify-between">
        <div class="flex items-center gap-0.5">
          <IconButton title="بزرگنمایی به" @click="actionWrapper(UnitActions.Zoom)">
      ...
      <TabPanel class="pt-4">
        <section class="relative" v-if="!isMultiMode">
          ...
          <div v-else-if="!isMultiMode" class="mb-4 space-y-4">
            <DescriptionItem label="نام">{{ unit.name }}</DescriptionItem>
            <DescriptionItem v-if="unit.shortName" label="نام کوتاه"
              >{{ unit.shortName }}
            </DescriptionItem>
            <DescriptionItem
              ...
              ></DescriptionItem
            >
            <DescriptionItem v-if="unit.description" label="توضیحات">
        ...
        <p v-else class="p-2 pt-4 text-sm">حالت ویرایش چندگانه هنوز پشتیبانی نمی‌شود.</p>
      </TabPanel>
      <TabPanel>
        <UnitDetailsSymbol
          :unit="unit"
...
const unitMenuItems = computed((): MenuItemData[] => [
  ...
  { label: "ویرایش داده‌های واحد", action: () => toggleEditMode(), disabled: isLocked.value },
  {
    label: "افزودن یا تغییر تصویر",
    action: () => toggleEditMediaMode(),
    disabled: isLocked.value,
  },
  { label: "حذف تصویر واحد", action: () => removeMedia(), disabled: isLocked.value },
  unit.value.locked
    ? {
        label: "باز کردن قفل واحد",
...
const buttonItems = computed(() => [
  ...
  {
    label: "تکثیر سلسله‌مراتب",
    onClick: () => actionWrapper(UnitActions.CloneWithSubordinates),
    disabled: isLocked.value,
  },
  {
    label: "تکثیر سلسله‌مراتب (با وضعیت)",
