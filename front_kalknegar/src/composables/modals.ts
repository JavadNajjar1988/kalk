...
export function useDateModal() {
  ...
  const dateModalTitle = ref("تنظیم زمان سناریو");
  ...
  const getModalTimestamp = async (
    ...
    dateModalTitle.value = options.title || "تنظیم زمان سناریو";
...
export function useSidcModal() {
  ...
  const sidcModalTitle = ref("انتخاب نماد");
  ...
  const getModalSidc = async (
    ...
    sidcModalTitle.value = options.title || "انتخابگر نماد";
