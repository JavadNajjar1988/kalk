...
export const SCENARIO_DB_VERSION = 4;
...
export async function useIndexedDb() {
  ...
  function createOrUpdateDb() {
    return openDB<ScenarioDb>("scenario-db", SCENARIO_DB_VERSION, {
      upgrade(db, oldVersion, newVersion, transaction) {
        ...
        if (oldVersion < 4 && !db.objectStoreNames.contains("scenario-drafts")) {
