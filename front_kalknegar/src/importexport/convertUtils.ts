...
export function addUnitHierarchy(
  ...
  const { state } = store;
  const tempUnitStatusIdMap = createNameToIdMapObject(state.unitStatusMap);
  const supplyNameToIdMap = createNameToIdMapObject(state.supplyCategoryMap);
  const equipmentNameToIdMap = createNameToIdMapObject(state.equipmentMap);
  const personnelNameToIdMap = createNameToIdMapObject(state.personnelMap);
  const sourceSupplyNameToIdMap = createNameToIdMapObject(
    sourceState?.supplyCategoryMap ?? {},
  );
  const targetSupplyUomNameToIdMap = createNameToIdMapObject(state.supplyUomMap);
  const targetSupplyClassMap = createNameToIdMapObject(state.supplyClassMap);
