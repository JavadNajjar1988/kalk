  ...
  if (hasZippedFileType(file)) {
    ...
    if (kmz) {
      ...
      guess.objectUrl = URL.createObjectURL(
        new Blob([Uint8Array.from(kmz[1]).buffer], { type: "application/vnd.google-earth.kml+xml" }),
      );
      Object.entries(unzipped).forEach(([filename, data]) => {
        if (!filename.endsWith(".kml")) {
          imageCache.set(filename, URL.createObjectURL(new Blob([Uint8Array.from(data).buffer])));
        }
      });
      return guess;
    }
