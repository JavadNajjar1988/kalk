...
let form = ref<Form>({ name: "طرف جدید", standardIdentity: "3", symbolOptions: {} });
...
<template>
  <InlineFormPanel @close="$emit('close')" title="ویرایش اطلاعات طرف">
    <form @submit.prevent="onFormSubmit" class="space-y-4">
      <InputGroup label="نام طرف" v-model="form.name" :id="focusId" />
      ...
      <div class="flex justify-end space-x-2">
        ...
        <Button variant="outline" size="sm" @click="$emit('close')">لغو</Button>
      </div>
    </form>
  </InlineFormPanel>
</template>
