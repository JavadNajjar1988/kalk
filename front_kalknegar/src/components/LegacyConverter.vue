<template>
  <div class="w-full space-y-4 p-1 pb-8">
    <p>تبدیل از و به کدهای شناسایی نماد مبتنی بر حرف قدیمی</p>
    <div class="grid grid-cols-4 content-center items-center gap-8">
      <p class="col-span-3">
        <InputGroup
          label="SIDC مبتنی بر حرف"
...
watch(
  ...
  (v) => {
    if (letterFocused.value || !numberFocused.value) {
      numberSidc.value = convertLetterSidc2NumberSidc(v).sidc || "مقدار نامشخص";
...
watch(numberSidcInput, (v) => {
  if (numberFocused.value) {
    letterSidc.value = convertNumberSidc2LetterSidc(v).sidc || "مقدار نامشخص";
