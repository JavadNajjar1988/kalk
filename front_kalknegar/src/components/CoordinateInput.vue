...
import { PhMapPin as MapPinIcon } from "@phosphor-icons/vue";

import * as mgrsLib from "mgrs";
import type { Position } from "geojson";
import { parseCoordinates, truncatePosition } from "@/geo/utils";
import { getErrorMessage, toPersianDigits, toEnglishDigits } from "@/utils";
...
watch([localValue, inputFormat], ([v, inpFormat], [oldV, oldInpFormat]) => {
  ...
  const englishValue = toEnglishDigits(v);

  if (inputFormat.value === "MGRS") {
    try {
      [lon, lat] = truncatePosition(mgrsLib.toPoint(englishValue) as Position);
    } catch (error) {
    ...
    try {
      const [first, second] = parseCoordinates(englishValue);
...
function convertToLocalValue(v: Position) {
  if (!v) return "";
  if (inputFormat.value === "MGRS") {
    const mgrsValue = mgrsLib.forward(v as [number, number], 5);
    return toPersianDigits(mgrsValue);
  } else {
    const [lon, lat] = truncatePosition(v);
    if (inputFormat.value === "LatLon") {
      return toPersianDigits(`${lat},${lon}`);
    } else {
      return toPersianDigits(`${lon},${lat}`);
    }
  }
}
  ...
  <div :class="outerClass" @blur.capture="onOuterBlur">
    <div class="relative rounded-sm shadow-xs">
      ...
      <div class="absolute inset-y-0 right-0 flex items-center">
        <label for="coordinate-format" class="sr-only">فرمت مختصات</label>
        <select
          ...
          ref="selectRef"
        >
          <option value="LatLon">عرض جغرافیایی، طول جغرافیایی</option>
          <option value="LonLat">طول جغرافیایی، عرض جغرافیایی</option>
