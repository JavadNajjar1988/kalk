...
const recordingSummary = computed(() =>
  activeRecordingLabels.value.length > 0 ? activeRecordingLabels.value.join("، ") : "خاموش",
);
...
<template>
  ...
  >
    <Button
      ...
      :aria-label="
        isAnyRecording
          ? `توقف ضبط. در حال ضبط: ${recordingSummary}.`
          : `ادامهٔ ضبط. آخرین انتخاب: ${recordingSummary}.`
      "
      :title="
        isAnyRecording
          ? `توقف ضبط. در حال ضبط: ${recordingSummary}.`
    ...
    <DropdownMenu>
      <DropdownMenuTrigger as-child>
        <Button
          ...
          aria-label="انتخاب حالت‌های ضبط"
          title="انتخاب حالت‌های ضبط"
        >
      ...
      <DropdownMenuContent align="end" class="w-56">
        <DropdownMenuLabel>تنظیمات ضبط</DropdownMenuLabel>
        <div class="text-muted-foreground px-2 py-1 text-xs">
          انتخاب کنید کدام تغییرات در زمان جاری سناریو ثبت شوند.
        </div>
        <DropdownMenuSeparator />
        <DropdownMenuLabel class="text-muted-foreground text-xs">واحدها</DropdownMenuLabel>

        <DropdownMenuCheckboxItem v-model="store.isRecordingHierarchy" @select.prevent>
          <span class="flex items-center gap-2">
            <Workflow class="h-4 w-4" />
            <span>سلسلهٔ واحدها</span>
          </span>
        </DropdownMenuCheckboxItem>
        <DropdownMenuCheckboxItem v-model="store.isRecordingLocation" @select.prevent>
          <span class="flex items-center gap-2">
            <MapPinned class="h-4 w-4" />
            <span>موقعیت واحد</span>
          </span>
        </DropdownMenuCheckboxItem>

        <DropdownMenuSeparator />
        <DropdownMenuLabel class="text-muted-foreground text-xs">فیچرها</DropdownMenuLabel>
