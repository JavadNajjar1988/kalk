...
import { createNameToIdMap, injectStrict, moveItemMutable, toPersianDigits } from "@/utils";
...
const stats = computed(() => {
  return {
    units: toPersianDigits(String(Object.keys(importedState.value.unitMap).length)),
  };
});
...
const computedColumns = computed((): (ColumnDef<Unit> | false)[] => {
  return [
    ...
    },
    {
      accessorFn: (f) =>
        f.state?.length
          ? toPersianDigits(fmt.trackFormatter.format(+new Date(f.state[f.state.length - 1].t))) +
            ` (${toPersianDigits(String(f.state.length))})`
          : "",
      id: "t",
      header: "آخرین ورودی وضعیت",
      enableSorting: false,
      size: 235,
    },
    {
  ...
  <div class="">
    <form @submit.prevent="onFormSubmit" class="mt-4 flex max-h-[80vh] flex-col">
      <div class="flex items-center justify-between">
        <p class="text-sm font-normal">سناریوی زیر را بارگذاری کرده‌اید:</p>
        <BaseButton secondary small @click="loadScenario(data)"
          >بارگذاری به عنوان سناریوی جدید
        </BaseButton>
      </div>
      ...
      <SimpleDivider class="mt-4">منبع</SimpleDivider>

      <PanelSubHeading class="mt-4"
        >کدام بخش‌های سناریو را می‌خواهید وارد کنید؟</PanelSubHeading
      >
      <div class="mt-2 flex items-center justify-between">
        <MRadioGroup class="flex w-full gap-5">
          <InputRadio v-model="importMode" value="side">طرف</InputRadio>
          <InputRadio v-model="importMode" value="group">گروه</InputRadio>
          <InputRadio v-model="importMode" value="equipment"
            >دسته‌بندی تجهیزات</InputRadio
          >
          <InputRadio v-model="importMode" value="personnel"
            >دسته‌بندی پرسنل</InputRadio
          >
          <InputRadio v-model="importMode" value="statuses">وضعیت‌های واحد</InputRadio>
          <InputRadio v-model="importMode" value="supplyCategories"
            >دسته‌بندی تأمین</InputRadio
          >
        ...
        <ToggleField
          ...
          disabled
          >انتخاب واحدهای فردی</ToggleField
        >
      </div>
      <template v-if="!isSettingsImport">
        <fieldset class="mt-4 flex w-full flex-col gap-4 sm:flex-row sm:items-center">
          ...
          <div v-else class="flex-auto self-end">
            <p v-if="hasExistingSide">این طرف در سناریوی هدف وجود دارد</p>
          </div>
        </fieldset>
        ...
        <fieldset>
          <InlineAlertWarning v-if="hasExistingUnits"
            >واحدهایی در سناریوی منبع وجود دارند که در سناریوی هدف وجود دارند.
          </InlineAlertWarning>
          <InlineAlertWarning v-if="importMode === 'group' && hasExistingSideGroup"
            >گروه انتخاب شده در سناریوی هدف وجود دارد.
          </InlineAlertWarning>
          <PanelSubHeading class="mt-4">چه چیزی را می‌خواهید وارد کنید؟</PanelSubHeading>
          <MRadioGroup class="mt-2 flex w-full items-center gap-5">
            ...
            <InputRadio
              ...
              value="state-only"
              >وضعیت (فقط برای واحدهای موجود)
            </InputRadio>
            <span v-else class="text-sm text-gray-500"
              >هیچ واحد منبعی در سناریوی هدف وجود ندارد.</span
            >
          </MRadioGroup>
        </fieldset>

        <SimpleDivider class="mt-4">هدف</SimpleDivider>
        <fieldset
          v-if="
        ...
        >
          <PanelSubHeading class="mt-4"
            >گروهی که می‌خواهید وارد کنید از قبل وجود دارد. چه کاری می‌خواهید انجام دهید؟
          </PanelSubHeading>
          <MRadioGroup class="mt-2 flex w-full gap-5">
            <InputRadio v-model="groupMergeMode" value="replace"
              >گروه موجود را بازنویسی کن
            </InputRadio>
            <InputRadio v-model="groupMergeMode" value="add_new"
              >به عنوان گروه جدید وارد کن
            </InputRadio>
          </MRadioGroup>
        </fieldset>
        <fieldset
        ...
        >
          <PanelSubHeading class="mt-4"
            >گزینه‌های وارد کردن وضعیت برای واحدهای موجود
          </PanelSubHeading>
          <MRadioGroup class="mt-2 flex w-full gap-5">
            <InputRadio v-model="stateMergeMode" value="replace"
              >وضعیت موجود را بازنویسی کن
            </InputRadio>
            <InputRadio v-model="stateMergeMode" value="add_new"
              >فقط وضعیت جدید را اضافه کن
            </InputRadio>
          </MRadioGroup>
        </template>
      </template>
      <template v-else-if="importMode === 'equipment'">
        <PanelSubHeading class="mt-4">دسته‌بندی تجهیزات</PanelSubHeading>
        <DataGrid
      ...
      <template v-else-if="importMode === 'personnel'">
        <PanelSubHeading class="mt-4">دسته‌بندی پرسنل</PanelSubHeading>
        <DataGrid
      ...
      <template v-else-if="importMode === 'statuses'">
        <PanelSubHeading class="mt-4">وضعیت‌های واحد</PanelSubHeading>
        <DataGrid
      ...
      <template v-else-if="importMode === 'supplyCategories'">
        <PanelSubHeading class="mt-4">دسته‌بندی تأمین</PanelSubHeading>
        <DataGrid
      ...
      >
        <ToggleField v-model="store.keepOpen">دیالوگ را باز نگه دار</ToggleField>
