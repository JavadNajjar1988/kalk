...
const page = (() => {
  ...
  return 'splash' // Default page if --page is not provided
})()

const projectUUID = page => {
...
const resolveServices = R.cond([
  ...
  [R.T, page => {
    const uuid = projectUUID(page)
    if (!uuid) return Promise.resolve(projectListServices())
    return Promise.resolve(projectServices(uuid))
  }]
])
