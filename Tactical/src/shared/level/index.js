...
// import leveldown from 'leveldown' // Node.js only - not used in browser
...
export const leveldb = (options = {}) => {
  ...
  if (options.down) return levelup(options.down)
    ...
    // In browser, always use memdown for in-memory storage
    // (level-js should be passed via options.down from outside)
    const down = memdown()
