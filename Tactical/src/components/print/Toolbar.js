  ...
  return (
    <header className='toolbar'>
      <div className='toolbar__items-container'>
        ...
        <DropDown options={['landscape', 'portrait'].map(orientation => ({ value: orientation, text: orientation === 'landscape' ? 'افقی' : 'عمودی' }))} selected={printSettings.orientation} onChange={changeHandler('orientation')}/>
        ...
        <input type='text' placeholder='عنوان اختیاری' maxLength={72} className='printToolbar_input' onChange={changeHandler('title')} value={printSettings.title}/>
