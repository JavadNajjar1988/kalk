...
import { useAmmunitionHierarchy, type AmmunitionPath, type AmmunitionFieldDefinition } from '@/hooks/useAmmunitionHierarchy';

interface AmmunitionHierarchicalSelectorProps {
  value?: AmmunitionPath[];
  onChange?: (path: AmmunitionPath[], finalNodeId?: string) => void;
  onFieldsChange?: (fields: AmmunitionFieldDefinition[]) => void;
  error?: string;
  disabled?: boolean;
}

const AmmunitionHierarchicalSelector: React.FC<AmmunitionHierarchicalSelectorProps> = ({
  value = [],
  onChange,
  onFieldsChange,
  error,
  ...
  const handleLevelChange = (level: number, nodeId: string) => {
    ...
    const selectedNode = options.find((node: any) => node.id === nodeId);
  ...
  const getCurrentValueForLevel = (level: number): string => {
    const pathItem = selectedPath.find((item: any) => item.level === level);
    ...
    return (
      ...
      >
        ...
        <Stack direction="row" spacing={1} flexWrap="wrap">
          {selectedPath.map((pathItem: any, index: number) => (
...
export default AmmunitionHierarchicalSelector;