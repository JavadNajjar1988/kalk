  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        <Box>
          <Typography variant="h5">وضعیت عملیاتی</Typography>
          <Typography variant="caption" color="text.secondary">
            وضعیت‌های مختلف عملیاتی یگان‌ها
          </Typography>
        </Box>
        <Button variant="outlined" color="primary" onClick={() => navigate(-1)} startIcon={<ArrowBackIcon />}>بازگشت</Button>
      </Box>
      <FieldsManagerBase categoryType={CategoryType.OPERATIONAL_STATUS} />
      <Box sx={{ mt: 3 }}>
        <HierarchyLevelsManagerBase categoryType={CategoryType.OPERATIONAL_STATUS} maxLevels={12} />
      </Box>
    </Box>
  );
};
