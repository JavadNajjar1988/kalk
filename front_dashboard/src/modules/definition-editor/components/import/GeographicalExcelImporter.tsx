...
interface GeographicalExcelImporterProps {
...
// Normalize a string for stable id creation
...
// Convert Excel rows with hierarchical columns to flat DefinitionNode[]
function convertRowsToNodes(rows: string[][], levelNames: string[]): DefinitionNode[] {
  ...
  const getOrCreateId = (pathSegments: string[]): string => {
    ...
    const newId = `geo_${slug || 'root'}_${idByPath.size + 1}`;
    idByPath.set(pathKey, newId);
    return newId;
  };

  for (const row of rows) {
    // Build path up to the last non-empty cell
    const trimmed = row.map(v => (v ?? '').toString().trim());
    let lastIdx = -1;
    for (let i = 0; i < trimmed.length; i++) {
      if (trimmed[i]) lastIdx = i;
    }
    if (lastIdx < 0) continue; // skip empty row
    ...
    for (let level = 0; level <= lastIdx; level++) {
      ...
      // Only push if not created before
...
const GeographicalExcelImporter: React.FC<GeographicalExcelImporterProps> = ({ levels, categoryId, categoryType, onImported }) => {
  ...
  const levelNames = useMemo(() => {
    // Use provided level names in order; fallback to generic labels
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      // Remove header if it matches level names; otherwise keep all
      const rows = json.filter(r => Array.isArray(r)) as (any[])[];
      // Normalize rows to strings and pad to requiredColumns
      ...
      // Try to detect header row if exact match with level names
      ...
      // Validate hierarchical gaps (no child without parent on previous level)
    ...
    <>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Typography variant="h6">ورود داده‌های جغرافیایی از اکسل</Typography>
...
export default GeographicalExcelImporter;
