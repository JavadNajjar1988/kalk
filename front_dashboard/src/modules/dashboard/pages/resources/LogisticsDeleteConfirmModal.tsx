...
import { Warning as WarningIcon, Delete as DeleteIcon, Cancel as CancelIcon, LocalShipping as LogisticsAvatarIcon } from '@mui/icons-material';
import { LogisticsItem } from '@/store/slices/tabularResourcesSlice';

interface LogisticsDeleteConfirmModalProps {
  open: boolean;
...
const LogisticsDeleteConfirmModal: React.FC<LogisticsDeleteConfirmModalProps> = ({ open, item, onClose, onConfirm, isDeleting = false }) => {
  ...
  return (
    <Dialog
      ...
      aria-labelledby="logistics-delete-dialog-title"
      aria-describedby="logistics-delete-dialog-description"
    >
      <DialogTitle
        id="logistics-delete-dialog-title"
        sx={{
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'center' }}>
          ...
          <Typography variant={isMobile ? 'h6' : 'h5'} fontWeight={700} color={(theme) => theme.palette.error.main}>
            تأیید حذف آیتم لجستیک
          </Typography>
        </Box>
      </DialogTitle>

      <DialogContent id="logistics-delete-dialog-description" sx={{ p: 0, backgroundColor: getSoftSurface() }}>
        <Box sx={{ p: isMobile ? 2 : 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 2, borderRadius: 2, bgcolor: (theme) => alpha(theme.palette.error.light, 0.06), border: (theme) => `1px solid ${alpha(theme.palette.error.light, 0.2)}` }}>
            <Avatar sx={{ width: 56, height: 56, bgcolor: (theme) => theme.palette.error.main }}>
              <LogisticsAvatarIcon sx={{ color: '#fff' }} />
            </Avatar>
            <Box>
              <Typography variant="body1" fontWeight={600}>{item.name}</Typography>
              <Typography variant="body2" color="text.secondary">کد آیتم: {item.itemCode}</Typography>
              {item.category && <Typography variant="body2" color="text.secondary">دسته‌بندی: {item.category}</Typography>}
            </Box>
          </Box>

          <Typography variant="body1" sx={{ mt: 3, mb: 1.5 }}>
            آیا مطمئن هستید که می‌خواهید این آیتم را حذف کنید؟ این عملیات غیرقابل بازگشت است.
          </Typography>
        </Box>
      </DialogContent>
...
export default LogisticsDeleteConfirmModal;
