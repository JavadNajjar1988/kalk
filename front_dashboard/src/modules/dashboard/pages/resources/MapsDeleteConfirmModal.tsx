...
import { Warning as WarningIcon, Delete as DeleteIcon, Cancel as CancelIcon, Map as MapAvatarIcon } from '@mui/icons-material';
...
interface MapsDeleteConfirmModalProps {
  open: boolean;
  mapItem: OfflineMapInfo | null;
  onClose: () => void;
  onConfirm: (id: number) => void;
  isDeleting?: boolean;
}

const MapsDeleteConfirmModal: React.FC<MapsDeleteConfirmModalProps> = ({ open, mapItem, onClose, onConfirm, isDeleting = false }) => {
  ...
  return (
    <Dialog
      ...
      aria-labelledby="offline-map-delete-dialog-title"
      aria-describedby="offline-map-delete-dialog-description"
    >
      <DialogTitle
        id="offline-map-delete-dialog-title"
        sx={{
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'center' }}>
          ...
          <Typography variant={isMobile ? 'h6' : 'h5'} fontWeight={700} color={(theme) => theme.palette.error.main}>
            تأیید حذف نقشه آفلاین
          </Typography>
        </Box>
      </DialogTitle>

      <DialogContent id="offline-map-delete-dialog-description" sx={{ p: 0, backgroundColor: getSoftSurface() }}>
        <Box sx={{ p: isMobile ? 2 : 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 2, borderRadius: 2, bgcolor: (theme) => alpha(theme.palette.error.light, 0.06), border: (theme) => `1px solid ${alpha(theme.palette.error.light, 0.2)}` }}>
            <Avatar sx={{ width: 56, height: 56, bgcolor: (theme) => theme.palette.error.main }}>
              <MapAvatarIcon sx={{ color: '#fff' }} />
            </Avatar>
            <Box>
              <Typography variant="body1" fontWeight={600}>{mapItem.name}</Typography>
              {mapItem.description && (
                <Typography variant="body2" color="text.secondary" noWrap sx={{ maxWidth: 360 }}>{mapItem.description}</Typography>
              )}
              {typeof mapItem.file_size === 'number' && (
                <Chip size="small" label={`${Math.round(mapItem.file_size / 1024 / 1024)} MB`} sx={{ mt: 0.5 }} />
              )}
            </Box>
          </Box>

          <Typography variant="body1" sx={{ mt: 3, mb: 1.5 }}>
            آیا مطمئن هستید که می‌خواهید این نقشه را حذف کنید؟ این عملیات غیرقابل بازگشت است.
          </Typography>
        </Box>
      </DialogContent>
      ...
      <DialogActions sx={{ backgroundColor: getSoftSurface(), borderTop: (theme) => `1px solid ${alpha(theme.palette.primary.light, 0.2)}`, p: isMobile ? 2 : 3, gap: 1.5 }}>
        ...
        <Button onClick={() => onConfirm(mapItem.id)} variant="contained" color="error" startIcon={<DeleteIcon />} disabled={isDeleting} sx={{ borderRadius: '12px' }}>
...
export default MapsDeleteConfirmModal;
