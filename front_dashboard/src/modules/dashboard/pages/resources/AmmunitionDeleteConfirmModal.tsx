...
import { Warning as WarningIcon, Delete as DeleteIcon, Cancel as CancelIcon, Security as AmmoAvatarIcon } from '@mui/icons-material';
import { AmmunitionItem } from '@/store/slices/tabularResourcesSlice';

interface AmmunitionDeleteConfirmModalProps {
  open: boolean;
  item: AmmunitionItem | null;
  onClose: () => void;
  onConfirm: (id: string) => void;
  isDeleting?: boolean;
}

const AmmunitionDeleteConfirmModal: React.FC<AmmunitionDeleteConfirmModalProps> = ({ open, item, onClose, onConfirm, isDeleting = false }) => {
  ...
  return (
    <Dialog
      ...
      aria-labelledby="ammunition-delete-dialog-title"
      aria-describedby="ammunition-delete-dialog-description"
    >
      <DialogTitle
        id="ammunition-delete-dialog-title"
        sx={{
      ...
      <DialogContent id="ammunition-delete-dialog-description" sx={{ p: 0, backgroundColor: getSoftSurface() }}>
        <Box sx={{ p: isMobile ? 2 : 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 2, borderRadius: 2, bgcolor: (theme) => alpha(theme.palette.error.light, 0.06), border: (theme) => `1px solid ${alpha(theme.palette.error.light, 0.2)}` }}>
            <Avatar sx={{ width: 56, height: 56, bgcolor: (theme) => theme.palette.error.main }}>
              <AmmoAvatarIcon sx={{ color: '#fff' }} />
            </Avatar>
            <Box>
              <Typography variant="body1" fontWeight={600}>{item.name}</Typography>
              <Typography variant="body2" color="text.secondary">کد مهمات: {item.ammunitionCode}</Typography>
              {item.caliber && <Typography variant="body2" color="text.secondary">کالیبر: {item.caliber}</Typography>}
            </Box>
          </Box>

          <Typography variant="body1" sx={{ mt: 3, mb: 1.5 }}>
            آیا مطمئن هستید که می‌خواهید این مهمات را حذف کنید؟ این عملیات غیرقابل بازگشت است.
          </Typography>
        </Box>
      </DialogContent>
...
export default AmmunitionDeleteConfirmModal;
