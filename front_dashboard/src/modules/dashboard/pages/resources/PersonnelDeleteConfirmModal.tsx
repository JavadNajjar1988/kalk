...
interface PersonnelDeleteConfirmModalProps {
  open: boolean;
...
const PersonnelDeleteConfirmModal: React.FC<PersonnelDeleteConfirmModalProps> = ({ open, person, onClose, onConfirm, isDeleting = false }) => {
  ...
  return (
    <Dialog
      ...
      aria-labelledby="personnel-delete-dialog-title"
      aria-describedby="personnel-delete-dialog-description"
    >
      <DialogTitle
        id="personnel-delete-dialog-title"
        sx={{
      ...
      <DialogContent id="personnel-delete-dialog-description" sx={{ p: 0, backgroundColor: getSoftSurface() }}>
        <Box sx={{ p: isMobile ? 2 : 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 2, borderRadius: 2, bgcolor: (theme) => alpha(theme.palette.error.light, 0.06), border: (theme) => `1px solid ${alpha(theme.palette.error.light, 0.2)}` }}>
            <Avatar sx={{ width: 56, height: 56, bgcolor: (theme) => theme.palette.error.main }}>{(person.firstName[0] || '').toUpperCase()}{(person.lastName[0] || '').toUpperCase()}</Avatar>
            <Box>
              <Typography variant="body1" fontWeight={600}>{person.firstName} {person.lastName}</Typography>
              <Typography variant="body2" color="text.secondary">کد پرسنلی: {person.personalCode}</Typography>
              {person.unit && <Typography variant="body2" color="text.secondary">یگان: {person.unit}</Typography>}
            </Box>
          </Box>

          <Typography variant="body1" sx={{ mt: 3, mb: 1.5 }}>
            آیا مطمئن هستید که می‌خواهید این نفر را حذف کنید؟ این عملیات غیرقابل بازگشت است.
          </Typography>
        </Box>
      </DialogContent>
      ...
      <DialogActions sx={{ backgroundColor: getSoftSurface(), borderTop: (theme) => `1px solid ${alpha(theme.palette.primary.light, 0.2)}`, p: isMobile ? 2 : 3, gap: 1.5 }}>
        ...
        <Button onClick={() => onConfirm(person.id)} variant="contained" color="error" startIcon={<DeleteIcon />} sx={{ borderRadius: '12px' }}>
          حذف
        </Button>
      </DialogActions>
    </Dialog>
...
export default PersonnelDeleteConfirmModal;
