...
const LogisticsModal: React.FC<LogisticsModalProps> = ({
  ...
  const handleLogisticsPathChange = (path: LogisticsPath[], _finalNodeId?: string) => {
          ...
          {logisticsHierarchyFields.length > 0 && (
            ...
            >
              ...
              >
                ...
                >
                  {t('resources.logistics.dynamicAttributes') ?? 'ویژگی‌های پویا'}
                </Typography>
                <Chip
                  label={`${logisticsHierarchyFields.length} ${t('common.fields') ?? 'فیلد'}`}
                  color="primary"
                  variant="outlined"
                  size="small"
                  sx={{ ml: 'auto' }}
              ...
              <Box sx={{ p: isMobile ? 2 : 3 }}>
                <Grid container spacing={2}>
                  {logisticsHierarchyFields.map((field) => (
                    <Grid
                      item
                      xs={12}
          ...
          {selectedLogisticsPath.length === 0 && (
            ...
            >
              <Typography variant="body2" color="info.main">
                لطفاً مسیر سلسله‌مراتب لجستیک را انتخاب کنید تا ویژگی‌های مرتبط فعال شوند.
              </Typography>
            </Paper>
          )}
        </Box>
