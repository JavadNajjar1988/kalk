...
import { formatPersianFileDate, formatPersianTime } from '@/utils/dateUtils';
import type {
  BaseMessage,
  CommandMessage,
  ResponseMessage,
...
export const OrbatDebugMonitor: React.FC = () => {
  ...
  useEffect(() => {
    const updateMetrics = () => {
      ...
      const avgLatency =
        commandEntries.reduce((sum, e) => sum + (e.duration || 0), 0) /
        Math.max(commandEntries.length, 1);
      ...
      const metrics: PerformanceMetric[] = [
        ...
        },
        {
          name: 'Error Rate',
          value: Math.round(
            (errorEntries.length / Math.max(recentEntries.length, 1)) * 100
          ),
          unit: '%',
          status:
  ...
  const exportDebugData = useCallback(() => {
    ...
    a.href = url;
    a.download = `orbat-debug-${formatPersianFileDate()}.json`;
    a.click();
    ...
    >
            ...
            <TableBody>
              {filteredEntries.map(entry => (
                <TableRow key={entry.id} hover>
                  <TableCell>
                    {formatPersianTime(entry.timestamp, {
                      second: '2-digit',
                    ...
                    {entry.data && (
                      <Tooltip title="View data">
                        <IconButton
                          size="small"
                          onClick={() =>
                            console.log('Debug entry data:', entry.data)
                          }
                        >
                          <Info />
                        </IconButton>
            ...
            {debugEntries.slice(0, 10).map(entry => (
              <ListItem key={entry.id}>
                ...
                <ListItemText
                  ...
                  secondary={`${entry.category} • ${formatPersianTime(
                    entry.timestamp,
                    {
                      second: '2-digit',
                    }
            ...
            <Card>
              <CardContent>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  ...
                  <Typography variant="body2">Timeout: 30s</Typography>
                  <Typography variant="body2">Retry Attempts: 3</Typography>
                  <Typography variant="body2">Heartbeat: Enabled</Typography>
                </Box>
              </CardContent>
            </Card>
          </Grid>
      ...
      <TabPanel value={activeTab} index={3}>
        <Alert severity="info" sx={{ mb: 2 }}>
          Cache monitoring features will be implemented when data service
          caching is active.
        </Alert>
      </TabPanel>
    </Paper>
