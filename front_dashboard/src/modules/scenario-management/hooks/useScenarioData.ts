...
const DEMO_SCENARIOS: DemoScenario[] = [
  {
    ...
    summary:
      'جنگ فالکلند یک درگیری نظامی بود که در سال ۱۹۸۲ بین آرژانتین و بریتانیا رخ داد. آرژانتین در ۲ آوریل ۱۹۸۲ به جزایر فالکلند حمله کرد و بریتانیا با اعزام نیروی ویژه برای بازپس‌گیری جزایر پاسخ داد.',
    imageUrl:
  ...
  },
  {
    ...
    summary:
      'مجموعه‌ای از درگیری‌های دریایی و زمینی بین نیروهای آلمان و متفقین از آوریل تا ژوئن ۱۹۴۰. این نبردها اولین پیروزی متفقین علیه آلمان در جنگ بود.',
    imageUrl:
...
export const useScenarioData = (
  options: UseScenarioDataOptions = {}
): UseScenarioDataResult => {
  ...
  const convertApiScenarios = useCallback(
    (apiData: any[]): ScenarioMetadata[] => {
      return apiData.map((scenario: any) => ({
        ...
        type:
          (scenario.metadata?.type as 'ORBAT-mapper' | 'custom') ??
          'ORBAT-mapper',
  ...
  const loadScenarios = useCallback(async () => {
    ...
    try {
      ...
      setState(prev => ({
        ...
        isLoading: false,
        error:
          error instanceof Error ? error.message : 'خطا در بارگذاری سناریوها',
      }));
    }
  }, [convertApiScenarios]);

  const handleScenarioAction = useCallback(
    async (action: ScenarioAction, scenarioId: string) => {
      try {
        switch (action) {
          case 'open':
          ...
          case 'run': {
            const simulatorBaseUrl =
              (import.meta as any).env?.VITE_SIMULATOR_URL ||
              'http://localhost:3001';
          ...
          case 'delete':
            if (
              window.confirm(
                'آیا مطمئن هستید که می‌خواهید این سناریو را حذف کنید؟'
              )
            ) {
          ...
          case 'duplicate':
            ...
            try {
              const duplicated =
                await scenarioApiService.duplicateScenario(scenarioId);
              await loadScenarios();
            } catch (error) {
  ...
  const handleDemoScenarioSelect = useCallback((scenarioId: string) => {
    ...
    window.open(
      `/kalknegar/scenario/demo-${scenarioId}?integration=react`,
      '_self'
    );
  }, []);

  const handleNewScenario = useCallback(() => {
    window.open('/dashboard/scenarios?create=1', '_self');
  }, []);

  const handleUploadScenario = useCallback(
  ...
  const handleLoadFromUrl = useCallback(
    async (url: string): Promise<UrlLoadResult> => {
      try {
        ...
        return {
          success: false,
          error:
            error instanceof Error ? error.message : 'خطا در بارگذاری از URL',
        };
      }
    },
    [convertApiScenarios, loadScenarios]
