...
### فایل‌های کاشی آفلاین (پوشه backend/static/maps/)
- پوشه `backend/static/maps/` به‌صورت پیش‌فرض در کانتینر API مانت می‌شود و انتظار می‌رود ساختار MapTiler chunk داشته باشد (`backend/static/maps/z{Z}/{X-group}/{Y-group}/{X-chunk}.{Y-chunk}.sqlitedb`).
